...
const Modal = ({title, children, toggle, setToggle, openedPermanently}) => {
  return (
    <div className='jmt-modal-container d-flex align-items-center justify-content-center' onClick={() => !openedPermanently && setToggle(false)} style={{ visibility: toggle ? 'visible' : 'hidden', opacity: toggle ? '1' : '0', transition: '.2s ease-in-out' }}>
